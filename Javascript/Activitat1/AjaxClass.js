...
class AjaxClass{
    ...
    carregarContingut (url, metode, params, funcioResposta, funcioState, funcioCapsalera, funcioCodi){
        //Si encara no hem inicialitzat la petició, ho fem ara...
        if(!_peticio_http){
            this.inicialitzarPeticio();
        }
        _peticio_http.onreadystatechange=() =>{
            funcioState(_peticio_http.readyState + "<br>");
            if(_peticio_http.readyState === READY_STATE_DONE) {
                funcioCodi(_peticio_http.status + " " + _peticio_http.statusText + "<br>");
              if(_peticio_http.status === SERVER_STATUS_OK) {
                  funcioResposta(_peticio_http.responseText);
                  funcioCapsalera(_peticio_http.getAllResponseHeaders());
              }
            }
        };
